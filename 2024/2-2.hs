...
safe list = almostPairs (\x y -> x < y && y - x <= 3) list || almostPairs (\x y -> x > y && x - y <= 3) list || and (zipWith (\x y -> x < y && y - x <= 3) (tail list) (tail $ tail list)) || and (zipWith (\x y -> x > y && x - y <= 3) (tail list) (tail $ tail list))  
